import{w as rl}from"./index.f9e74c88.js";/**
 * @license
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const Gy=5*60,Wy=ua("authIdTokenMaxAge")||Gy;let sa=null;const Qy=n=>async e=>{const t=e&&await e.getIdTokenResult(),s=t&&(new Date().getTime()-Date.parse(t.issuedAtTime))/1e3;if(s&&s>Wy)return;const i=t==null?void 0:t.token;sa!==i&&(sa=i,await fetch(n,{method:i?"POST":"DELETE",headers:i?{Authorization:`Bearer ${i}`}:{}}))};function Xy(n=ji()){const e=$i(n,"auth");if(e.isInitialized())return e.getImmediate();const t=Qg(n,{popupRedirectResolver:qy,persistence:[Jm,Bm,Xu]}),s=ua("authTokenSyncURL");if(s){const r=Qy(s);Mm(t,r,()=>r(t.currentUser)),Lm(t,o=>r(o))}const i=ca("auth");return i&&vm(t,`http://${i}`),t}zy("Browser");const ia={apiKey:"AIzaSyBOyg_n3Nvn7mJkwrSIm6Jl57URTrDh-P0",authDomain:"svelte-todo-fullstack.firebaseapp.com",projectId:"svelte-todo-fullstack",storageBucket:"svelte-todo-fullstack.appspot.com",messagingSenderId:"304474384073",appId:"1:304474384073:web:52ac0dcb617579d5494f18"};let ut;Ah().length&&(ut=ji(),kh(ut)),ut=di(ia);const nv=Ng(ut),oi=Xy(ut),sv=rl({user:null,loading:!0,data:{}}),iv={signup:async(n,e)=>{await Om(oi,n,e)},login:async(n,e)=>{await Pm(oi,n,e)},logout:async()=>{await xm(oi)}};export{tv as J,ev as K,Zy as Z,oi as a,sv as b,iv as c,nv as d};
